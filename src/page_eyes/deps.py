...
SimulateDeviceType: TypeAlias = (
    Literal["iPhone 15", "iPhone 15 Pro", "iPhone 15 Pro Max", "iPhone 6"] | str
)
...
class ToolParams(BaseModel):
    ...
    action: str = Field(
        description="要执行的动作名称，如 click、input、swipe、open_app 等, 操作调用的工具名称"
    )
...
class LLMLocationToolParams(ToolParams):
    ...
    def get_coordinate(
        ...
        bbox = ctx.deps.context.current_step.screen_elements[self.element_id].get(
            "bbox"
        )
...
class VLMLocationToolParams(ToolParams):
    coordinate: tuple[int, int, int, int] = Field(
        description="要操作的元素坐标，格式为(x1, y1, x2, y2)"
    )
    element_content: str = Field(description="要操作的元素内容")
...
LocationToolParams = (
    VLMLocationToolParams
    if default_settings.model_type == "vlm"
    else LLMLocationToolParams
)
...
class ClickToolParams(LocationToolParams):
    ...
    position: Optional[PositionType] = Field(
        default=None, description="点击元素的相对位置"
    )
...
class SwipeToolParams(ToolParams):
    ...
    repeat_times: Optional[int] = Field(
        default=1, description="重复次数或最多重复次数，默认为1次"
    )
...
class SwipeForKeywordsToolParams(SwipeToolParams):
    ...
    expect_keywords: Optional[list[str]] = Field(
        default=None, description="期望出现的关键字列表"
    )


class SwipeFromCoordinateToolParams(ToolParams):
    coordinates: conlist(item_type=tuple[int, int], min_length=2) = Field(
        description="滑动起始坐标列表"
    )
