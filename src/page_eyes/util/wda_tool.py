        ...
        try:
            ...
            if apps_info:
                for bundle_id, app_info in apps_info.items():
                    display_name = app_info.get("CFBundleDisplayName") or app_info.get(
                        "CFBundleName", ""
                    )
                    app_list.append(
                        AppInfo(bundle_id=bundle_id, display_name=display_name)
                    )
            logger.info(f"Found {len(app_list)} apps on device using pymobiledevice3")

        except ImportError:
            logger.warning(
                "pymobiledevice3 not installed, falling back to WDA app_list"
            )
        except Exception as e:
            logger.warning(
                f"Failed to get app list with pymobiledevice3: {e}, falling back to WDA"
            )
